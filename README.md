...
missing required fields, and duplicate event delivery — before they reach
production.

## What's here

- **`app/main.py`** — a minimal `POST /track` endpoint (FastAPI) that stands
  in for a real conversion-tracking server. It validates incoming events and
  rejects duplicate `event_id`s.
...
curl -X POST http://localhost:8000/track \
  ...
  -d '{"event_id": "evt-001", "event_type": "purchase", "timestamp": "2026-09-11T12:00:00Z"}'
...
`.github/workflows/test.yml` installs dependencies and runs `pytest` on every
pull request targeting `main`, so a regression in validation or
deduplication behavior fails the PR check instead of reaching production.
